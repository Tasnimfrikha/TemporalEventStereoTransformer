...
def convbn(in_planes, out_planes, kernel_size, stride, pad, dilation):
    return nn.Sequential(
        ...
        nn.GroupNorm(num_groups=4, num_channels=out_planes)  # Use GroupNorm instead of BatchNorm2d
    )

def convbn_3d(in_planes, out_planes, kernel_size, stride, pad):
    return nn.Sequential(
        nn.Conv3d(in_planes, out_planes, kernel_size=kernel_size, padding=pad, stride=stride, bias=False),
        nn.GroupNorm(num_groups=4, num_channels=out_planes)  # Use GroupNorm
    )
...
class feature_extraction(nn.Module):
    def __init__(self, in_channels=15, height=64, width=64):
    ...
    def _make_layer(self, block, planes, blocks, stride, pad, dilation):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.GroupNorm(num_groups=4, num_channels=planes * block.expansion)  # Use GroupNorm
            )
        layers = []
